...
con = mysql.connector.connect(
    host="localhost", user="root", password="", database="employee") # Enter Your MySQL password (password="ENTER YOUR PASSWORD") then run the file.

# Assume regex patterns for email and phone number are defined
regex = r'^[a-z0-9]+[\._]?[a-z0-9]+[@]\w+[.]\w+$'  # Email validation regex
Pattern = re.compile(r'^[0-9]{10}$')  # Phone number validation pattern
...
def Update_Employ():
    ...
    if(check_employee(Id) == False):
        ...
        # Collecting new data for update
        Name = input("Enter Employee Name: ")
        ...
        # SQL Query to update all fields in empdata Table
        sql = '''
        UPDATE empdata 
        SET Name = %s, Email_Id = %s, Phone_no = %s, Address = %s, Post = %s, Salary = %s 
        WHERE Id = %s
        '''
        data = (Name, Email_Id, Phone_no, Address, Post, Salary, Id)
        c = con.cursor()
...
def Export_To_PDF():
    print("{:>60}".format("-->> Export Employee Record to PDF <<--"))
    ...
    # Adding a title
    pdf.cell(200, 10, txt="Employee Records", ln=True, align='C')

    # Adding column headers
    headers = ['ID', 'Name', 'Email', 'Phone', 'Address', 'Post', 'Salary']
    ...
    for emp in employees:
        for field in emp:
            pdf.cell(30, 10, str(field), border=1)
        pdf.ln()

    # Save the PDF
    pdf_file = 'Employee_Records.pdf'
    pdf.output(pdf_file)
    print(f"PDF Exported Successfully as {pdf_file}")
    press = input("Press Any Key To Continue..")
    menu()

# Function to generate Excel file
def Export_To_Excel():
    print("{:>60}".format("-->> Export Employee Record to Excel <<--"))
    ...
    wb.save(excel_file)
    print(f"Excel Exported Successfully as {excel_file}")
    press = input("Press Any Key To Continue..")
    menu()
...
def menu():
    ...
    print("7. Export Employee Data to PDF")
    print("8. Export Employee Data to Excel")
    print("9. Exit\n")
    ...
    if ch == 1:
        ...
        system("cls")
        print("{:>60}".format("Thanks For Using Employee Management System"))
        exit(0)
    else:
